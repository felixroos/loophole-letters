...
const createFaustNode = async (audioContext) => {
  ...
  const dspModule = await WebAssembly.compileStreaming(
    await fetch("./dspModule.wasm")
  );
  /** @type {FaustDspDistribution} */
  const faustDsp = { dspMeta, dspModule };
  console.log(faustDsp);
  try {
...
(async () => {
  const {
    ...
    voices,
    dspMeta: { name },
  } = await createFaustNode(audioContext);
  ...
  if (voices && navigator.requestMIDIAccess)
    await buildMidiDeviceMenu(faustNode);
